FurnaceUITooltip = ISPanel:derive("FurnaceUITooltip");

local tooltipCache = {}
local packItemsCache = {}

function FurnaceUITooltip:initialise()
	ISPanel.initialise(self);
end

function FurnaceUITooltip:instantiate()
	ISPanel.instantiate(self)
	self.javaObject:setConsumeMouseEvents(false)
end

function FurnaceUITooltip:setItem(item)
	self.item = item;
end

function FurnaceUITooltip:onMouseDown(x, y)
	return false
end

function FurnaceUITooltip:onMouseUp(x, y)
	return false
end

function FurnaceUITooltip:onRightMouseDown(x, y)
	return false
end

function FurnaceUITooltip:onRightMouseUp(x, y)
	return false
end

function FurnaceUITooltip:prerender()
...
function FurnaceUITooltip:render()
	...
	if tooltip then
		local y = 50
		local x = 20
		self:drawText(FurnaceUIText.Contains..":", x-10, y-25, 1, 1, 1, 1, UIFont.Small)
		local quantity = nil
		...
		if self.item.drop then
			self:drawText(FurnaceUIText.DropOnFloor, x-15, y+5, 1, 1, 1, 1, UIFont.Small)
		end
		return
	end
...
function FurnaceUITooltip:doLayout()
	...
	if self.item.drop then
		local dropWidth = getTextManager():MeasureStringX(UIFont.Small, FurnaceUIText.DropOnFloor)
...
function FurnaceUITooltip:setOwner(ui)
	self.owner = ui
end
...
function FurnaceUITooltip:reset()
    self:setVisible(false)
    self:noBackground()
    self.footNote = nil
